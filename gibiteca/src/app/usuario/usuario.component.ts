...
export class UsuarioComponent implements OnInit {
  ...
  ngOnInit() {
    //this.usuarioService.getAll().subscribe(data => this.usuarios = data, err => {alert('Aconteceu um erro ' + err) ;});
    this.novo();
    ...
    this.usuarios = [{
      'codigo':1,
      'login':'cavanhaman@hotmail.com',
      'senha':'123456',
    }]
  }

  pesquisarTodos() {
    this.usuarioService.getAll().subscribe(data =>this.usuarios = data, err =>{alert('Aconteceu um erro!'); 
  });}

  /********* */
  novo() {this.usuario = new Usuario();}

  cancelar() {this.voltar();}

  voltar() {this.router.navigate(['/usuario']);}

  salvar() {
    let result;
    if (this.isNew) {result = this.usuarioService.add(this.usuario);}
    else {result = this.usuarioService.update(this.usuario);}
    ...
    if (this.usuario.codigo == null) {alert('Selecione algum usuario');}
      ...
      if (confirm('Você realmente quer excluir o usuario '+this.usuario.nome+'?')) {
        ...
        .subscribe(
          ...
          err => {alert('Usuario não removido');});
          this.novo();
          this.voltar();
      }
    }
